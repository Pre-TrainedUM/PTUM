...
for i in userdata:
    ...
    for j in userdata[i][1]:
        ...
        for m in j[0]:
            if m in word_dict :
                wordidtitle.append(word_dict[m][0])

        wordidtitle=wordidtitle[:MAXTL]
        wordid.append(wordidtitle+[0]*(MAXTL-len(wordidtitle)))
    wordid=wordid[:MAXT]
    user_titlepos.append(list(range(1,len(wordid)+1))+[0]*(MAXT-len(wordid)))
    user_title.append(wordid+[[0]*MAXTL]*(MAXT-len(wordid)))
    lefttime=[0]+[int(np.log2(userdata[i][1][x+1][1]-userdata[i][1][x][1]+1)) for x in range(len(userdata[i][1][:100])-1)]
    righttime=lefttime[1:]+[0]  
    righttime+=[0]*(100-len(wordid))
...
for i in ads:
    tp=i.replace('\n','').split('\t')
    wordidtitle=[word_dict[j][0] for j in word_tokenize(tp[1]) if j in word_dict][:30]
    wordiddes=[word_dict[j][0] for j in word_tokenize(tp[2]) if j in word_dict][:30]
    adsdict[tp[0]]=[wordidtitle+[0]*(30-len(wordidtitle)),wordiddes+[0]*(30-len(wordiddes))]  
    ...
    for j in title:
        ...
        for m in j:
            if m in  word_dict :
                ...
                wordidtitle.append(1)
        wordidtitle=wordidtitle[:MAXTL]
        wordid.append(wordidtitle+[0]*(MAXTL-len(wordidtitle)))
    wordid=wordid[:MAXT]
    wordid=wordid+[[0]*MAXTL]*(MAXT-len(wordid))
    ttpos=list(range(1,len(wordid)+1))+[0]*(MAXT-len(wordid))
